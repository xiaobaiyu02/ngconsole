...
wsgate.RDP = new Class( {
    ...
    initialize: function(url, canvas, cssCursor, useTouch, vkbd, rdphost) {
        ...
        $('#keyboardlanguage')[0].addEvent('click', this.ToggleLanguageButton.bind(this));
    },
    Disconnect: function() {
        this._reset();
    },
    ...
    ToggleLanguageButton: function(){
        if(this.useIME){
            this.useIME = false;
            $('#keyboardlanguage')[0].removeClass('extracommandshold');
        }else{
            this.useIME = true;
            $('#keyboardlanguage')[0].addClass('extracommandshold');
        }
    },
    /**
    ...
    SetupCanvas: function(canvas){
        ...
        this.textAreaInput = document.createElement('input');
        ...
        this.textAreaInput.type = 'password';
        this.textAreaInput.autocomplete = false;
    ...
    _pmsg: function(data) { // process a binary RDP message from our queue
        ...
        switch (op[0]) {
            ...
            case 8:
                ...
                if (this.cssC) {
                    this.cursors[hdr[0]] = (this.msie > 0 || this.trident > 0) ? 'url('+ this.domain +'/cur/' + this.sid + '/' + hdr[0] + '), none' : //IE is not suporting given hot spots
                                            'url('+ this.domain +'/cur/' + this.sid + '/' + hdr[0] + ') ' + hdr[1] + ' ' + hdr[2] + ',none'; 
                } else {
                    this.cursors[hdr[0]] = (this.msie > 0 || this.trident > 0) ? { u: this.domain +'/cur/' + this.sid + '/' + hdr[0] } :
                                            { u: this.domain +'/cur/' + this.sid + '/' + hdr[0], x: hdr[1], y: hdr[2] };
                }
                break;
            case 9:
                // PTR_FREE
    ...
    onMouseLeave: function(evt){
       for(var button in this.mouseDownStatus){
           if(this.mouseDownStatus[button]){
               ...
               var maxX = $('#textareainput')[0].getStyle('width').toInt();
               var maxY = $('#textareainput')[0].getStyle('height').toInt();
               if (x < 0) x = 0;
               if (y < 0) y = 0;
               if (x > maxX) x = maxX;
               if (y > maxY) y = maxY;
     ...
     * up = 0
     */
    SendKeyUpDown: function(key, upDown, location){
        if (this.sock.readyState == this.sock.OPEN) {
            buf = new ArrayBuffer(16);
            a = new Uint32Array(buf);
    ...
    KeyDownEvent: function(evt){
        if(!this.useIME){
            this.SendKeyUpDown(evt.code, 1, evt.event.location);
        }else{
    ...
    KeyUpEvent: function(evt){
        if(!this.useIME){
            this.SendKeyUpDown(evt.code, 0, evt.event.location);
            this.textAreaInput.set("value","");
        }else{
            if(!this.IMEon)
            if(this.FunctionalKey(evt.code)){
    ...
    onWSmsg: function(evt) {
        ...
        if ($('#dvLoading')[0].getStyle("visibility") !== "hidden") {
            $('#dvLoading')[0].setStyles({ 'visibility': 'hidden' });
        }
            ...
            case 'string':
                ...
                switch (evt.data.substr(0,2)) {
                    ...
                    case 'R:':
                            ...
                            $('#screen')[0].width=resolution[0];
                            $('#screen')[0].height=resolution[1];
                            this.bstore.width=resolution[0];
                            this.bstore.height=resolution[1];
                            $('#textareainput')[0].setStyle('width', resolution[0]+'px');
                            $('#textareainput')[0].setStyle('height', resolution[1]+'px');
                            break;
                    case 'C:':
    ...
    onWSerr: function (evt) {
        this.open = false;
        console.log(88888, this.sock)
        if(this.sock.readyState == 3){
            this.fireEvent('tokenTimeout', true);
        };
...
function refreshIMEhelper(){
    ...
    $('#IMEhelper')[0].setStyle('visibility','visible');
    // IME helper div
   //  console.log(222222, rdp)
   //  if(rdp && rdp.IMEon){
   //      $('#IMEhelper')[0].setStyle('visibility','visible');
   //      $('#IMEhelper')[0].set('html',$('#textareainput')[0].get('value'));
   //  }else{
   //      $('#IMEhelper')[0].setStyle('visibility','hidden');
   // }
}
