...
angular.module("vdi.rdp", ["app.controllers", "app.localize", "ngResource", "ui.bootstrap", "vdi", "vdi.desktop", "vdi.template", "vdi.template.teach", "vdi.template.person", "vdi.template.hardware", "vdi.system", "vdi.user"]).config(["$locationProvider", function ($locationProvider) {
    $locationProvider.html5Mode(true);
}])
// .controller("isoListController", ["$scope", "$modalInstance", "SystemISO", "VNC", function($scope, $modalInstance, isos, vnc){
//     $scope.$on('disconnectRdp', function($event, data){
//         console.log(data)
//     })
//     $scope.isos = [];
...
.controller("updateTemplateController", ["$scope", "$modalInstance", "SystemISO", function ($scope, $modalInstance, isos) {
...
}]).controller("rdpController", ["$scope", "VMCommon", "VNC", "TeachTemplate", "PersonTemplate", "SystemISO", "Admin", "PersonDesktop", "$modal", "$timeout", "Scene", "$$$os_types", "UserRole", "$$$I18N", function ($scope, vm, vnc, template, personTemplate, iso, admin, desktop, $modal, $timeout, scen, $$$os_types, UserRole, $$$I18N) {
    var wsBase = "";
    ...
    function popUpMessage(type, msg, timeout, callback, center) {
        ...
        newMessage.addEvent('mousedown', function () {
        ...
        newMessage.setStyle('background-color', 'rgba(' + color.r + ',' + color.g + ',' + color.b + ', 0.8)');
    ...
    var sendDisconnect = function sendDisconnect() {
        if (confirm('Are you sure you want to disconnect ?')) {
            rdp.Disconnect();
    ...
    function showDialog(show) {
        if (show) {
            ...
            $('#maindialog')[0].addClass('invisible');
            $('#ctrlaltdelete')[0].addEvent('click', function () {
                rdp.SendKey(0);
            });
    ...
    function domready(wsUrl, host) {
        ...
        var wsOK = RIMtablet || webkitOK || Browser.firefox && Browser.version >= 11.0 || Browser.chrome && Browser.version >= 17 || Browser.safari && Browser.version >= 6 || Browser.ie && Browser.version >= 10.0;
...
/******/ });
//# sourceMappingURL=templateModifyRdp.map