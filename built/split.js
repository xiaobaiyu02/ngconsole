...
angular.module("vdi.split", ["ngRoute", "ngResource", "ui.bootstrap", "app.localize", "app.controllers", "vdi"]).run(['localize', function (localize) {
    localize.setLang();
...
}]).controller("splitBrainController", ["$scope", "localize", "$$$I18N", "$timeout", "$location", "$rootScope", "Split", '$Domain', "$splitDomain", function ($scope, localize, $$$I18N, $timeout, $location, $rootScope, Split, domain, $splitDomain) {
    ...
    function getLists() {
        Split.split_brain_info(function (res) {
            $scope.loading = false;
            $scope.servers = res.result.sort(function (a, b) {
